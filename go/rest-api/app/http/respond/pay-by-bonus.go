...
type (
	// payByBonusResponse describes response.
	payByBonusResponse struct {
...
func (r *payByBonusResponse) Render(_ http.ResponseWriter, rq *http.Request) error {
	render.Status(rq, r.Code)
	return nil
}

// PayByBonusResource wraps resource to respond.
func PayByBonusResource(rs http.ResponseWriter, rq *http.Request, rp *repo.DocumentRepo) {
	// init calculate response
	r := &payByBonusResponse{
		repo:     rp,
		Document: resp.NewDocument(rp),
	}

	// @FIXME fake response text
	text := "Pay by bonus response"
